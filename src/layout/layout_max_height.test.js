...
it('sets the max height at so the element is not larger than the body', () => {
  ...
  jest.spyOn(listbox, 'getBoundingClientRect').mockImplementation(() => ({ top: 50 }));
...
it('does not set a negative max-height', () => {
  jest.spyOn(document.body, 'getBoundingClientRect').mockImplementation(() => ({ bottom: 100, height: 0 }));
  jest.spyOn(document.documentElement, 'clientHeight', 'get').mockImplementation(() => 125);
  jest.spyOn(listbox, 'getBoundingClientRect').mockImplementation(() => ({ top: 130 }));
...
describe('when there is insufficent space for the list box', () => {
  it('moves the listbox above', () => {
    jest.spyOn(document.body, 'getBoundingClientRect').mockImplementation(() => ({ height: 150, bottom: 150, top: 0 }));
    jest.spyOn(document.body, 'clientHeight', 'get').mockImplementation(() => 140);
    jest.spyOn(document.documentElement, 'clientHeight', 'get').mockImplementation(() => 125);
    jest.spyOn(listbox, 'getBoundingClientRect').mockImplementation(() => ({ top: 50, height: 50 }));
    jest.spyOn(input, 'getBoundingClientRect').mockImplementation(() => ({ bottom: 120, top: 100 }));
  ...
  describe('when contained', () => {
    it('sets the max height correctly', () => {
      jest.spyOn(document.body, 'getBoundingClientRect').mockImplementation(() => ({ bottom: 150 }));
      jest.spyOn(document.documentElement, 'clientHeight', 'get').mockImplementation(() => 125);
      jest.spyOn(container, 'getBoundingClientRect').mockImplementation(() => ({ top: 10, bottom: 100, height: 0 }));
      jest.spyOn(listbox, 'getBoundingClientRect').mockImplementation(() => ({ top: 50, height: 50 }));
      jest.spyOn(input, 'getBoundingClientRect').mockImplementation(() => ({ bottom: 120, top: 100 }));
