...
describe('options', () => {
  ...
  describe('options as undefined', () => {
    ...
    it('triggers the onValue callback with the selected value', async () => {
      const spy = jest.fn();
      render(<ComboBoxWrapper value="foo" options={['foo', undefined]} onValue={spy} />);
      await userEvent.tab();
      await userEvent.keyboard('{ArrowDown}{Enter}');
...
describe('value', () => {
  ...
  describe('value is null', () => {
    ...
    it('selects the value', async () => {
      render(<ComboBoxWrapper options={options} value={null} />);
      await userEvent.tab();
      expectToHaveSelectedOption(screen.getAllByRole('option')[0]);
    });
  });
    ...
    describe('backspace', () => {
      ...
      describe('ctrl+d', () => {
        it('continues to auto-select an option', async () => {
          ...
          await userEvent.type(screen.getByRole('combobox'), 'foo{Control>}d{/Control}');
    ...
    describe('delete', () => {
      ...
      describe('ctrl+k', () => {
        it('continues to auto-select an option', async () => {
          ...
          await userEvent.type(screen.getByRole('combobox'), 'foo{Control>}k{/Control}');
...
describe('notFoundMessage', () => {
  ...
  describe('with custom message', () => {
    it('displays custom not found if search returns no results', async () => {
      render((
        <ComboBoxWrapper options={[]} notFoundMessage={() => 'custom message'} />
      ));
      await userEvent.type(screen.getByRole('combobox'), 'foo');
...
describe('aria live message', () => {
  ...
  it('adds a debounced message', async () => {
    ...
    expect(await collectLiveMessages(async () => {
      await user.tab();
...
describe('aria-describedby', () => {
  it('is appended to the input', async () => {
    ...
    await userEvent.tab();
    expect(screen.getByRole('combobox')).toHaveAttribute('aria-describedby', 'foo id_aria_description');
  });
});
...
describe('renderNotFound', () => {
  it('allows the not found message to be replaced', async () => {
    render(
      <ComboBoxWrapper options={['foo']} renderNotFound={(props) => <div data-testid="notfound" {...props} />} />,
    );
...
describe('onLayoutListBox', () => {
  ...
  it('is not called when the component is rendered', async () => {
