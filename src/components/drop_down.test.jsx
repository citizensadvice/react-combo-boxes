...
describe('value', () => {
  ...
  describe('updating the value', () => {
    it('updates the aria-selected value of an open listbox', async () => {
      ...
      expect(document.querySelector('[aria-selected="true"]')).toEqual(
        screen.getByRole('option', { name: 'Apple' }),
      );
    });
...
describe('aria-invalid', () => {
  it('is not set with no value', () => {
    render(<DropDownWrapper options={['one', 'two']} />);
...
describe('renderListBoxWrapper', () => {
  it('allows the list box wrapper to be replaced', () => {
    ...
    expect(
      screen.getByRole('listbox', { hidden: true }).parentNode.tagName,
    ).toEqual('DL');
    expect(
      screen.getByRole('listbox', { hidden: true }).parentNode,
